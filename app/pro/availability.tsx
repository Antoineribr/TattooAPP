    ...
    <View style={{ flex: 1, backgroundColor: "#0A0A0B" }}>
      <View style={{ paddingTop: 56, paddingHorizontal: 20, paddingBottom: 16, flexDirection: "row", alignItems: "center", gap: 12 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ color: "#F4F1EA", fontSize: 20, fontWeight: "800" }}>Disponibilités</Text>
        </View>
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20, gap: 16 }}>
        ...
        {loading ? (
          ...
          <View style={{ gap: 10, marginBottom: 8 }}>
            <Text style={{ color: "#F4F1EA", fontWeight: "700", fontSize: 14 }}>Statuts actifs</Text>
            {availabilities.map((av) => (
              <View key={av.id} style={{ backgroundColor: "#17171A", borderRadius: 14, padding: 14, flexDirection: "row", alignItems: "center", gap: 12 }}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ color: "#F4F1EA", fontWeight: "700", fontSize: 13 }}>{AVAILABILITY_STATUS_LABELS[av.status]}</Text>
                  {av.note && <Text style={{ color: "rgba(244,241,234,0.55)", fontSize: 12, marginTop: 2 }}>{av.note}</Text>}
                </View>
                <TouchableOpacity onPress={() => handleDelete(av.id)}>
                  <Ionicons name="close-circle-outline" size={22} color="rgba(244,241,234,0.28)" />
                </TouchableOpacity>
              </View>
            ))}
          </View>
          ...
          <View style={{ alignItems: "center", paddingVertical: 20 }}>
            <Text style={{ color: "rgba(244,241,234,0.55)", fontSize: 14 }}>Aucun statut actif. Ajoute-en un ci-dessous.</Text>
          </View>
        )}

        {/* Ajouter un statut */}
        <Text style={{ color: "#F4F1EA", fontWeight: "700", fontSize: 14 }}>Ajouter un statut</Text>
        <View style={{ gap: 8 }}>
          {(Object.keys(STATUS_ICONS) as ArtistAvailabilityStatus[]).map((s) => {
            ...
            return (
              <TouchableOpacity
                ...
                disabled={isActive}
                style={{ flexDirection: "row", alignItems: "center", gap: 10, padding: 13, borderRadius: 12, backgroundColor: isActive ? "#F0F0F0" : isSelected ? "rgba(184,144,62,0.1)" : "#17171A", borderWidth: 1.5, borderColor: isSelected && !isActive ? "#C9A24B" : "transparent", opacity: isActive ? 0.5 : 1 }}
              >
                <Ionicons name={STATUS_ICONS[s] as any} size={18} color={isActive ? "#999" : STATUS_COLORS[s]} />
                <View style={{ flex: 1 }}>
                  <Text style={{ color: isActive ? "#999" : "#F4F1EA", fontWeight: "600", fontSize: 13 }}>{AVAILABILITY_STATUS_LABELS[s]}</Text>
                  {isActive && <Text style={{ color: "#27AE60", fontSize: 11 }}>Déjà actif</Text>}
                </View>
                {isSelected && !isActive && <Ionicons name="checkmark-circle" size={18} color="#C9A24B" />}
              </TouchableOpacity>
            );
          })}
        </View>
        ...
        <TextInput
          ...
          style={{ backgroundColor: "#17171A", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, color: "#F4F1EA", fontSize: 14, borderWidth: 1, borderColor: "rgba(244,241,234,0.08)" }}
        />

        <TouchableOpacity
          onPress={handleAdd}
          disabled={saving || activeStatuses.includes(selectedStatus)}
          style={{ backgroundColor: activeStatuses.includes(selectedStatus) ? "rgba(244,241,234,0.06)" : "#C9A24B", borderRadius: 14, paddingVertical: 14, alignItems: "center" }}
        >
          {saving ? <ActivityIndicator color="#FFF" /> : <Text style={{ color: activeStatuses.includes(selectedStatus) ? "rgba(0,0,0,0.3)" : "#0A0A0B", fontWeight: "700", fontSize: 15 }}>Activer ce statut</Text>}
        </TouchableOpacity>
