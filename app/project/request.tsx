    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: "#0A0A0B" }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      <View style={{ paddingTop: 56, paddingHorizontal: 20, paddingBottom: 16 }}>
        <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 16 }}>
          ...
          <Text style={{ flex: 1, color: "#F4F1EA", fontWeight: "700", fontSize: 16, textAlign: "center" }}>
            Demander un projet
          </Text>
          <View style={{ width: 26 }} />
        </View>
        ...
        <View style={{ flexDirection: "row", gap: 6 }}>
          {STEPS.map((_, i) => (
            <View key={i} style={{ flex: 1, height: 3, borderRadius: 2, backgroundColor: i <= step ? "#C9A24B" : "rgba(244,241,234,0.1)" }} />
          ))}
        </View>
        <Text style={{ color: "rgba(244,241,234,0.55)", fontSize: 12, marginTop: 6 }}>
      ...
      <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 20, paddingBottom: 120 }}>
        {/* Artiste cible */}
        <View style={{ flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "#17171A", borderRadius: 12, padding: 12, marginBottom: 24 }}>
          <Ionicons name="color-palette-outline" size={16} color="#C9A24B" />
          <Text style={{ color: "#F4F1EA", fontWeight: "600" }}>Pour {artistName ?? "ce tatoueur"}</Text>
          {postImage && (
        ...
        {step === 0 && (
          <View style={{ gap: 12 }}>
            <Text style={{ color: "#F4F1EA", fontSize: 20, fontWeight: "800", marginBottom: 8 }}>Quel type de demande ?</Text>
            {([
              ...
              <TouchableOpacity
                ...
                style={{
                  ...
                  backgroundColor: requestType === t.value ? "rgba(184,144,62,0.1)" : "#17171A",
                  borderRadius: 14, padding: 16,
                  borderWidth: 1.5, borderColor: requestType === t.value ? "#C9A24B" : "transparent",
                }}
              >
                <View style={{ width: 44, height: 44, borderRadius: 12, backgroundColor: requestType === t.value ? "rgba(184,144,62,0.2)" : "rgba(244,241,234,0.08)", alignItems: "center", justifyContent: "center" }}>
                  <Ionicons name={t.icon} size={22} color={requestType === t.value ? "#C9A24B" : "#6B6B7A"} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: requestType === t.value ? "#C9A24B" : "#F4F1EA", fontWeight: "700", fontSize: 15 }}>{t.label}</Text>
                  <Text style={{ color: "rgba(244,241,234,0.55)", fontSize: 13, marginTop: 2 }}>{t.desc}</Text>
                </View>
                {requestType === t.value && <Ionicons name="checkmark-circle" size={22} color="#C9A24B" />}
              </TouchableOpacity>
            ))}
          </View>
        )}
        ...
        {step === 1 && (
          <View>
            <Text style={{ color: "#F4F1EA", fontSize: 20, fontWeight: "800", marginBottom: 4 }}>Décris ton idée</Text>
            <Text style={{ color: "rgba(244,241,234,0.55)", fontSize: 14, marginBottom: 20 }}>Plus tu es précis·e, mieux le tatoueur pourra te répondre.</Text>
            ...
            <TextInput
              ...
              style={{
                ...
                color: "#F4F1EA", fontSize: 15, minHeight: 140, borderWidth: 1, borderColor: "rgba(244,241,234,0.1)",
                textAlignVertical: "top",
              }}
            />

            <Text style={{ color: description.trim().length >= 20 ? "#6B6B7A" : "#C9A24B", fontSize: 12, marginTop: 8, marginBottom: 20 }}>
              {description.trim().length < 20
            ...
            {/* Photos de référence */}
            <Text style={{ color: "#F4F1EA", fontWeight: "700", fontSize: 15, marginBottom: 10 }}>
              Photos de référence <Text style={{ color: "rgba(244,241,234,0.55)", fontWeight: "400" }}>(optionnel, max 5)</Text>
            </Text>
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
              ...
              {refImages.length < 5 && (
                <TouchableOpacity
                  onPress={pickImages}
                  style={{ width: 80, height: 80, borderRadius: 10, backgroundColor: "#17171A", borderWidth: 1, borderColor: "rgba(244,241,234,0.1)", borderStyle: "dashed", alignItems: "center", justifyContent: "center" }}
                >
        ...
        {step === 2 && (
          <View>
            <Text style={{ color: "#F4F1EA", fontSize: 20, fontWeight: "800", marginBottom: 4 }}>Informations</Text>
            <Text style={{ color: "rgba(244,241,234,0.55)", fontSize: 14, marginBottom: 20 }}>
            ...
            {/* Recherche */}
            <View style={{ flexDirection: "row", alignItems: "center", backgroundColor: "#17171A", borderRadius: 12, paddingHorizontal: 12, paddingVertical: 9, gap: 8, borderWidth: 1, borderColor: "rgba(244,241,234,0.08)", marginBottom: 12 }}>
              ...
              <TextInput
                ...
                style={{ flex: 1, color: "#F4F1EA", fontSize: 14 }}
              />
              {placementSearch !== "" && (
                <TouchableOpacity onPress={() => setPlacementSearch("")}>
                  <Ionicons name="close-circle" size={16} color="rgba(244,241,234,0.28)" />
                </TouchableOpacity>
              )}
            </View>
            {/* Sélection actuelle */}
            {bodyPlacement !== "" && (
              ...
              >
                <Ionicons name="checkmark-circle" size={15} color="#C9A24B" />
                <Text style={{ color: "#C9A24B", fontWeight: "700", fontSize: 13 }}>{bodyPlacement}</Text>
                <Ionicons name="close" size={13} color="#C9A24B" />
              </TouchableOpacity>
            )}
            {/* Catégories */}
            <View style={{ gap: 14, marginBottom: 20 }}>
              {Object.entries(BODY_PLACEMENTS_FULL).map(([category, placements]) => {
                ...
                return (
                  <View key={category}>
                    <Text style={{ color: "rgba(244,241,234,0.55)", fontSize: 11, fontWeight: "700", letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>{category}</Text>
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                      {filtered.map((bp) => (
                        <TouchableOpacity
                          key={bp}
                          onPress={() => setBodyPlacement(bodyPlacement === bp ? "" : bp)}
                          style={{ paddingHorizontal: 13, paddingVertical: 7, borderRadius: 20, backgroundColor: bodyPlacement === bp ? "#C9A24B" : "#17171A", borderWidth: 1, borderColor: bodyPlacement === bp ? "#C9A24B" : "rgba(244,241,234,0.1)" }}
                        >
                          <Text style={{ color: bodyPlacement === bp ? "#0A0A0B" : "#F4F1EA", fontSize: 13, fontWeight: "600" }}>{bp}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  </View>
            ...
            <View style={{ flexDirection: "row", gap: 8, marginBottom: 20 }}>
              {([["any", "Indifférent"], ["black_grey", "Noir & gris"], ["color", "Couleur"]] as const).map(([val, lbl]) => (
                <TouchableOpacity
                  ...
                  style={{ flex: 1, paddingVertical: 10, borderRadius: 12, alignItems: "center", backgroundColor: colorPref === val ? "#C9A24B" : "#17171A", borderWidth: 1, borderColor: colorPref === val ? "#C9A24B" : "rgba(244,241,234,0.1)" }}
                >
                  <Text style={{ color: colorPref === val ? "#0A0A0B" : "#F4F1EA", fontWeight: "600", fontSize: 13 }}>{lbl}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
              {STYLES_LIST.map((s) => (
                <TouchableOpacity
                  ...
                  style={{ paddingHorizontal: 13, paddingVertical: 7, borderRadius: 20, backgroundColor: desiredStyle === s ? "#C9A24B" : "#17171A", borderWidth: 1, borderColor: desiredStyle === s ? "#C9A24B" : "rgba(244,241,234,0.1)" }}
                >
                  <Text style={{ color: desiredStyle === s ? "#0A0A0B" : "#F4F1EA", fontSize: 13, fontWeight: "600" }}>{s}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={{ flexDirection: "row", gap: 10, marginBottom: 20 }}>
              <TextInput value={budgetMin} onChangeText={setBudgetMin} placeholder="Min" placeholderTextColor="rgba(244,241,234,0.28)" keyboardType="number-pad"
              ...
              <TextInput value={budgetMax} onChangeText={setBudgetMax} placeholder="Max" placeholderTextColor="rgba(244,241,234,0.28)" keyboardType="number-pad"
                style={{ flex: 1, backgroundColor: "#17171A", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, color: "#F4F1EA", fontSize: 15, borderWidth: 1, borderColor: "rgba(244,241,234,0.1)" }} />
            </View>

            <FieldLabel label="Période souhaitée" />
            <TextInput value={desiredDate} onChangeText={setDesiredDate} placeholder="Ex : mars 2026, après Pâques…" placeholderTextColor="rgba(244,241,234,0.28)"
              style={{ backgroundColor: "#17171A", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, color: "#F4F1EA", fontSize: 15, borderWidth: 1, borderColor: "rgba(244,241,234,0.1)", marginBottom: 20 }} />

            <FieldLabel label="Ta ville" />
            <TextInput value={city} onChangeText={setCity} placeholder="Ex : Paris, Lyon…" placeholderTextColor="rgba(244,241,234,0.28)"
              style={{ backgroundColor: "#17171A", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, color: "#F4F1EA", fontSize: 15, borderWidth: 1, borderColor: "rgba(244,241,234,0.1)", marginBottom: 20 }} />
          </View>
        )}
        ...
        {step === 3 && (
          <View>
            <Text style={{ color: "#F4F1EA", fontSize: 20, fontWeight: "800", marginBottom: 20 }}>Récapitulatif</Text>
            ...
            <View style={{ backgroundColor: "rgba(184,144,62,0.1)", borderRadius: 12, padding: 14, marginTop: 12, borderWidth: 1, borderColor: "rgba(184,144,62,0.2)" }}>
              <Text style={{ color: "#C9A24B", fontSize: 13, lineHeight: 20 }}>
      ...
      {/* Bouton */}
      <View style={{ position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "#0A0A0B", borderTopWidth: 1, borderTopColor: "#17171A", padding: 20, paddingBottom: 40 }}>
        <TouchableOpacity
          onPress={step < 3 ? () => setStep(s => s + 1) : handleSend}
          disabled={!canGoNext || sending}
          style={{
            backgroundColor: canGoNext ? "#C9A24B" : "rgba(244,241,234,0.06)",
            borderRadius: 14, paddingVertical: 16, alignItems: "center",
          }}
        >
          {sending ? <ActivityIndicator color="#0A0A0B" /> : (
            <Text style={{ color: canGoNext ? "#0A0A0B" : "rgba(244,241,234,0.28)", fontWeight: "800", fontSize: 16 }}>
...
function FieldLabel({ label }: { label: string }) {
  return <Text style={{ color: "rgba(244,241,234,0.55)", fontSize: 12, fontWeight: "700", letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 }}>{label}</Text>;
}

function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <View style={{ flexDirection: "row", paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: "#17171A", gap: 12 }}>
      <Text style={{ color: "rgba(244,241,234,0.55)", fontSize: 14, width: 90 }}>{label}</Text>
      <Text style={{ color: "#F4F1EA", fontSize: 14, flex: 1, fontWeight: "500" }}>{value}</Text>
    </View>
  );
}
