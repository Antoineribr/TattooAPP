  ...
  return (
    <View style={{ flex: 1, backgroundColor: pageBg }}>
      ...
      <View style={{ paddingTop: desktop ? 28 : 56, paddingHorizontal: desktop ? 28 : 16, paddingBottom: desktop ? 16 : 10, backgroundColor: pageBg, borderBottomWidth: desktop ? 1 : 0, borderBottomColor: softBorder }}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", backgroundColor: panelBg, borderRadius: 14, paddingHorizontal: 14, paddingVertical: 10, gap: 8, borderWidth: 1, borderColor: softBorder, marginBottom: 8 }}>
          <Ionicons name="search" size={16} color={secondaryText} />
          <TextInput value={search} onChangeText={setSearch} placeholder="Ville, nom d'artiste…" placeholderTextColor={desktop ? "rgba(244,241,234,0.28)" : "rgba(0,0,0,0.2)"} style={{ flex: 1, color: primaryText, fontSize: 14 }} />
          {search !== "" && <TouchableOpacity onPress={() => setSearch("")}><Ionicons name="close-circle" size={16} color={secondaryText} /></TouchableOpacity>}
        </View>
        ...
        {availableNow.length > 0 && (
          <View style={{ marginBottom: 10 }}>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 8 }}>
              ...
              <Text style={{ color: primaryText, fontSize: 12, fontWeight: "700" }}>Dispo maintenant</Text>
              <Text style={{ color: secondaryText, fontSize: 12 }}>· {availableNow.length} tatoueur{availableNow.length > 1 ? "s" : ""}</Text>
            </View>
                ...
                {[...availableNow].sort((a, b) => {
                  ...
                  <TouchableOpacity key={a.id} onPress={() => router.push(`/profile/${a.id}`)} style={{ alignItems: "center", gap: 5 }}>
                    <View style={{ position: "relative" }}>
                      ...
                      <View style={{ position: "absolute", bottom: 1, right: 1, width: 12, height: 12, borderRadius: 6, backgroundColor: "#4CAF50", borderWidth: 2, borderColor: pageBg }} />
                    </View>
                    <Text style={{ color: primaryText, fontSize: 11, fontWeight: "600", maxWidth: 56, textAlign: "center" }} numberOfLines={1}>{a.display_name.split(" ")[0]}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </ScrollView>
          ...
          {[...filtered].sort((a, b) => {
            ...
            return (
              <TouchableOpacity key={a.id} onPress={() => router.push(`/profile/${a.id}`)}
                style={{ backgroundColor: panelBg, borderRadius: 16, padding: 14, flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, borderColor: softBorder }}>
                <View style={{ position: "relative" }}>
                  <Avatar uri={a.avatar_url} name={a.display_name} size={54} />
                  {dispo && <View style={{ position: "absolute", bottom: 0, right: 0, width: 13, height: 13, borderRadius: 7, backgroundColor: "#4CAF50", borderWidth: 2, borderColor: pageBg }} />}
                </View>
                <View style={{ flex: 1 }}>
                ...
                <View style={{ alignItems: "flex-end", gap: 4 }}>
                  ...
                  {a.starting_price != null && (
                    <Text style={{ color: secondaryText, fontSize: 12 }}>dès {a.starting_price}€</Text>
                  )}
                </View>
              </TouchableOpacity>
            );
      ...
      {/* Map */}
      <View style={{ flex: 1, display: viewMode === "map" ? "flex" : "none", margin: desktop ? 18 : 0, marginTop: desktop ? 16 : 0, borderRadius: desktop ? 22 : 0, overflow: "hidden", borderWidth: desktop ? 1 : 0, borderColor: softBorder }}>
        {isWeb ? (
          <iframe
        ...
        {selected && (
          <TouchableOpacity
            ...
            style={{ position: "absolute", bottom: desktop ? 16 : (Platform.OS === "ios" ? 90 : 70), left: 16, right: 16, backgroundColor: panelBg, borderRadius: 18, padding: 14, flexDirection: "row", alignItems: "center", gap: 12, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 12, borderWidth: desktop ? 1 : 0, borderColor: softBorder }}
          >
